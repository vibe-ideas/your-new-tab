...
The popup is split into three tabs so you no longer have to scroll through every option:

- **Bookmarks** — choose between built-in bookmarks or pasted JSON.
- **Search** — manage the default search provider and the list shown in the new tab.
- **Backgrounds** — paste animated image/video direct links.

A persistent action footer (Save / Reset / Test / Refresh bookmarks) sits below the tab content and stays visible no matter which tab is active. Switching tabs preserves any edits in memory, so you can move between them without losing work.

| Bookmarks tab | Search tab | Backgrounds tab |
| :---: | :---: | :---: |
| ![Bookmarks tab](./popup-tab-bookmarks.png) | ![Search tab](./popup-tab-search.png) | ![Backgrounds tab](./popup-tab-backgrounds.png) |

> Screenshots are generated by `node e2e/popup-screenshots.mjs` against the built extension.

## Bookmark configuration

The **Bookmarks** tab keeps two source options:
...
2. Switch to the **Backgrounds** tab.
3. Paste one direct media link per line in the **Animated background media links (one per line)** field.
4. Click **Save** in the persistent footer.
